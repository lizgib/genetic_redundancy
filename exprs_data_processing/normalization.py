...
df = pd.read_csv(sys.argv[1], sep='\t', index_col = 1, header=0) #dataframe to normalize
print('found the expression file')
print(df.head())
...
#    newdf= df.loc[rows_to_norm,:].apply(normalize_values, axis=0, result_type= 'expand') # this line threw error for the statement 'result_type = expand so removing it for now '
    newdf= df.loc[rows_to_norm,:].apply(normalize_values, axis=0)
else:
...
newdf.to_csv(sys.argv[3], sep="\t", header=True)

# this thing below may not work...
fig1 = plt.figure(2, figsize= (20,8))
df.boxplot()
fig1.savefig('outputs/figures/before_norm_boxplt.png')

fig2 = plt.figure(2, figsize= (20,8))
newdf.boxplot()
fig2.savefig('outputs/figures/normalized_boxplt.png')
